...
class MongoDBClient:
    ...
    def seed_demo_data(self):
        ...
        print("🌱 Seeding dataset data into MongoDB Atlas...")
        ...
        try:
            ...
            if profiles and transactions:
                ...
                for start in range(0, len(transactions), 500):
                    txn_col.insert_many(transactions[start:start + 500])
                txn_col.create_index("account_id")

                print(f"✅ Seeded {len(profiles)} profiles + {len(transactions)} transactions into MongoDB Atlas")
                return
        except Exception as exc:
            print(f"⚠️  Dataset MongoDB seed failed, using legacy fallback: {exc}")

        print("🌱 Seeding legacy demo data into MongoDB Atlas...")
    ...
    def get_customer_profile(self, account_id: str) -> dict:
        ...
        if self._use_simulator:
            from infrastructure.databases.simulators import dynamodb_sim
