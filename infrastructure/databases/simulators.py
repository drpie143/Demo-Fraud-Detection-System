...
class DynamoDBSimulator:
    ...
    - Kiểm tra account age và device/IP patterns
    """

    def __init__(self):
        # --- Customer Profiles ---
        # Mỗi profile chứa thông tin behavioral baseline + device/IP
        self._profiles: dict[str, dict] = {
            "ACC_001": {
                ...
                "risk_category": "low",
                "device_id": "fe14f7ec-e97c-4acf-84d5-ca177182da22",
                "ip_address": "111.122.183.48",
                "geolocation": {"lat": 20.820068, "long": 108.494128},
            },
            "ACC_007": {
                "customer_id": "ACC_007",
                "name": "Trần Thị B",
                ...
                "device_id": "60936c71-21b7-4971-8ee1-986df801d914",
                "ip_address": "57.238.150.188",
                "geolocation": {"lat": 8.747506, "long": 108.21475},
            },
            "ACC_050": {
                "customer_id": "ACC_050",
                "name": "Unknown Entity",
                "account_type": "business",
                "account_age_days": 15,           # Rất mới → RED FLAG
                "avg_monthly_transactions": 0,    # Chưa có lịch sử
                "avg_transaction_amount": 0.0,
                "typical_channels": ["web"],
                "typical_locations": ["Unknown"],
                "risk_category": "high",
                "device_id": "fc09fe3f-77d6-4709-90b0-67877bd90e29",
                "ip_address": "47.135.65.245",
                "geolocation": {"lat": 8.826032, "long": 104.47419},
            },
            "MULE_001": {
                ...
                "risk_category": "high",
                "device_id": "2e222c33-b565-42a6-99da-00219c77613f",
                "ip_address": "214.38.213.98",
                "geolocation": {"lat": 20.158374, "long": 105.997599},
            },
            "ACC_002": {
                "customer_id": "ACC_002",
                "name": "Trần Minh Tuấn",
                ...
                "risk_category": "low",
                "device_id": "998116a3-9a62-4143-8550-20ce96d09331",
                "ip_address": "109.16.241.127",
                "geolocation": {"lat": 9.414442, "long": 106.004524},
            },
            "ACC_666": {
                "customer_id": "ACC_666",
    ...
    def get_customer_profile(self, account_id: str) -> dict:
        """
        Lấy customer profile (behavioral baseline + device/IP).
        ...
        self._documents: list[dict] = [
            {
                ...
                "description": (
                    "Nhiều giao dịch nhỏ ngay dưới ngưỡng cấu hình của dataset "
                    "được thực hiện trong thời gian ngắn để tránh trigger cảnh báo tự động. "
                    "Tín hiệu mạnh là hành vi lặp lại qua nhiều tài khoản nhận."
                ),
                "indicators": [
                    "Số tiền ngay dưới ngưỡng cấu hình",
            ...
            },
            {
                ...
                "title": "Case: Dataset Fraud Cluster C2972777054",
                "description": (
                    "Dataset account C2972777054 gửi giao dịch gian lận đã biết "
                    "đến C8992641070 với amount 27000. Cụm liên quan có fraud_ratio cao, "
                    "shared infrastructure và hành vi balance-drain lặp lại."
                ),
                "related_accounts": ["C2972777054", "C8992641070", "C2006456468", "C3259274595"],
                "decision": "BLOCK",
                "date": "2026-03-24",
            },
            {
                ...
                "title": "Dataset-Scale Threshold Policy",
                "description": (
                    "Demo dùng threshold từ runtime config vì final.csv có scale PaySim/VND-like. "
                    "High value nên đưa vào review, không auto-block nếu thiếu bằng chứng fraud khác."
                ),
...
class RedisService:
    ...
    def get_amount_thresholds(self) -> dict:
        if self._mode == "real":
            ...
            return {
                "instant_allow_max": 5000,
                "escalate_threshold": 5000,
                "instant_block_threshold": 10000000,
                "currency": "VND",
            }
    ...
    def seed_data(self):
        ...
        r = self._real_redis
        print("   📊 Seeding Redis Cloud data from dataset...")

        try:
            from configs.settings import settings
            from infrastructure.databases.seed_loader import load_processed_seed_data

            data = load_processed_seed_data()
            profiles = data.get("profiles", [])
            raw_edges = data.get("raw_edges", [])

            if profiles and raw_edges:
                pipe = r.pipeline(transaction=False)
                pipe.delete("blacklist")

                for pattern in ["account:*", "whitelist:*", "risk_score:*", "velocity:*", "rules:*"]:
                    keys = r.keys(pattern)
                    if keys:
                        pipe.delete(*keys)
                ...
                receivers_by_sender: dict[str, dict[str, int]] = {}

                for edge in raw_edges:
                    sender = str(edge.get("sender_account_no") or "")
                    receiver = str(edge.get("receiver_account_no") or "")
                    if not sender:
                        continue
                    sender_counts[sender] = sender_counts.get(sender, 0) + 1
                    if receiver:
                        receivers_by_sender.setdefault(sender, {})
                        receivers_by_sender[sender][receiver] = receivers_by_sender[sender].get(receiver, 0) + 1

                for profile in profiles:
                    acc_id = profile.get("customer_id")
                    if not acc_id:
                        continue
                    ...
                    pipe.hset(f"account:{acc_id}", mapping={
                        ...
                        "country": "VN",
                        "status": "blocked" if risk_category == "critical" or fraud_ratio >= 0.8 else "active",
                    })

                    if risk_category == "critical" or fraud_ratio >= 0.8:
                        pipe.sadd("blacklist", acc_id)
                    elif risk_category == "low" and profile.get("avg_monthly_transactions", 0):
                        receivers = sorted(
                            receivers_by_sender.get(acc_id, {}).items(),
                        ...
                        if receivers:
                            pipe.hset(
                                f"whitelist:{acc_id}",
                                mapping={receiver: str(90 - index * 10) for index, (receiver, _) in enumerate(receivers)},
                            )

                    score = max(risk_map.get(risk_category, 0.3), min(0.98, 0.2 + fraud_ratio * 0.78))
                    pipe.hset(f"risk_score:{acc_id}", mapping={
                ...
                pipe.hset("rules:velocity", mapping={
                    ...
                    "max_transactions_per_day": "20",
                    "max_amount_per_day": "250000000",
                    "description": "Dataset-scale velocity rules",
                })
                pipe.hset("rules:amount_threshold", mapping={
                    ...
                    "currency": "VND",
                    "description": "Dataset-scale amount thresholds",
                })
                pipe.execute()
                print(f"      Dataset Redis seed complete: {len(profiles)} profiles")
                return
        except Exception as exc:
            print(f"      Dataset Redis seed failed, using legacy fallback: {exc}")

        print("   📊 Seeding Redis Cloud legacy fallback data...")
