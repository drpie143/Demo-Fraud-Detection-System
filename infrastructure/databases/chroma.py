...
class VectorStore:
    ...
    def __init__(self):
        ...
        if settings.demo_mode:
            print("ℹ️  DEMO_MODE=true → dùng OpenSearchSimulator")
            self.client = None
    ...
    def seed_knowledge_base(self):
        ...
        documents = [
            ...
            {
                "id": "case_dataset_mule_cluster",
                "text": (
                    "Dataset mule cluster case: C2972777054 -> C8992641070 and "
                    "C2006456468 -> C3259274595 are known fraud-cluster transfers. "
                    "Strong signals include high fraud ratio, repeated transfers, "
                    "shared device/IP evidence, and balance-drain behavior."
                ),
                "metadata": {
                    "type": "past_investigation",
                    "decision": "BLOCK",
                    "title": "Dataset Mule Cluster Case",
            ...
            },
            {
                ...
                "text": (
                    "Dataset-Scale Threshold Policy: final.csv uses PaySim/VND-like "
                    "amounts, so review thresholds come from runtime settings. "
                    "Structuring around those thresholds is a signal to investigate. "
                    "High value alone should review, not automatically block."
                ),
                "metadata": {
    ...
    def search(self, query: str, top_k: int = 3, filter_type: Optional[str] = None) -> list[dict]:
        ...
        if not self.collection or self.collection.count() == 0:
            ...
            from infrastructure.databases.simulators import opensearch_sim
