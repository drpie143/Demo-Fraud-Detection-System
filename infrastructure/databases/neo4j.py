...
#
# SCHEMA:
#   (:Account {id, name, risk, type, account_age_days})
#   (:Device {id, label})
#   (:IP {id, label, is_vpn})
...
class Neo4jClient:
    ...
    def __init__(self):
        ...
        if settings.demo_mode:
            print("ℹ️  DEMO_MODE=true → dùng NeptuneSimulator")
            return
    ...
    def seed_demo_data(self):
        ...
        print("🌱 Seeding dataset graph into Neo4j AuraDB...")

        try:
            from infrastructure.databases.seed_loader import load_processed_seed_data, _safe_float
            ...
            if profiles and raw_edges:
                with self.driver.session() as session:
                    ...
                    devices = {
                        str(edge.get("device_id"))
                        for edge in raw_edges
                        if edge.get("device_id")
                    }
                    ips = {
                        str(edge.get("ip_address"))
                        for edge in raw_edges
                        if edge.get("ip_address")
                    }
                    session.run(
                        "UNWIND $devices AS d CREATE (:Device {id: d.id, label: d.label})",
                        devices=[{"id": device, "label": f"Device-{device[:8]}"} for device in devices],
                    )
                    session.run(
                        "UNWIND $ips AS ip CREATE (:IP {id: ip.id, label: ip.label})",
                        ips=[{"id": ip, "label": ip} for ip in ips],
                    )
                    ...
                    for edge in raw_edges:
                        ...
                        if not sender or not receiver:
                            continue
                        key = (sender, receiver)
                        stats = transfer_map.setdefault(key, {"amount": 0.0, "count": 0})
                        stats["amount"] += _safe_float(edge.get("amount"))
                        stats["count"] += 1

                    transfers = [
                        {"src": src, "dst": dst, "amount": round(stats["amount"], 2), "count": stats["count"]}
                        for (src, dst), stats in transfer_map.items()
                    ]
                    session.run(
                        """
                        UNWIND $transfers AS t
                        MATCH (a:Account {id: t.src}), (b:Account {id: t.dst})
                        CREATE (a)-[:TRANSFERS_TO {total_amount: t.amount, count: t.count}]->(b)
                        """,
                        transfers=transfers,
                    )

                    device_edges = [
                        {"account": str(edge.get("sender_account_no")), "device": str(edge.get("device_id"))}
                        for edge in raw_edges
                        if edge.get("sender_account_no") and edge.get("device_id")
                    ]
                    ip_edges = [
                        {"account": str(edge.get("sender_account_no")), "ip": str(edge.get("ip_address"))}
                        for edge in raw_edges
                        if edge.get("sender_account_no") and edge.get("ip_address")
                    ]
                    session.run(
                        """
                        UNWIND $edges AS e
                        MATCH (a:Account {id: e.account}), (d:Device {id: e.device})
                        MERGE (a)-[:USES_DEVICE {since: 'dataset'}]->(d)
                        """,
                        edges=device_edges,
                    )
                    session.run(
                        """
                        UNWIND $edges AS e
                        MATCH (a:Account {id: e.account}), (ip:IP {id: e.ip})
                        MERGE (a)-[:CONNECTS_FROM {frequency: 'dataset'}]->(ip)
                        """,
                        edges=ip_edges,
                    )

                print(f"✅ Seeded {len(profiles)} accounts and {len(transfers)} transfer edges into Neo4j AuraDB")
                return
        except Exception as exc:
            print(f"⚠️  Dataset Neo4j seed failed, using legacy fallback: {exc}")

        print("🌱 Seeding legacy demo data into Neo4j AuraDB...")
        ...
        with self.driver.session() as session:
            ...
            # ─── Tạo Account nodes ───
            session.run("""
                CREATE (:Account {id: 'ACC_001', name: 'Nguyễn Văn An', risk: 'low', type: 'savings', account_age_days: 1825})
                CREATE (:Account {id: 'ACC_002', name: 'Trần Minh Tuấn', risk: 'low', type: 'checking'})
                CREATE (:Account {id: 'ACC_007', name: 'Trần Thị B', risk: 'medium', type: 'checking', account_age_days: 45})
                ...
                CREATE (:Account {id: 'ACC_050', name: 'Unknown Entity', risk: 'high', type: 'business', account_age_days: 15})
    ...
    def get_neighbors(self, node_id: str, depth: int = 2) -> dict:
        ...
        if self._use_simulator:
            from infrastructure.databases.simulators import neptune_sim
    ...
    def ingest_transaction(self, sender_id: str, receiver_id: str, amount: float,
        ...
        Also creates Device and IP nodes if provided.
        """
        if self._use_simulator:
            from infrastructure.databases.simulators import neptune_sim
        ...
        with self.driver.session() as session:
            ...
            session.run("""
                ...
                ON CREATE SET t.total_amount = coalesce(t.total_amount, 0) + $amount, t.count = coalesce(t.count, 0) + 1
            ...
            if device_id:
                session.run("""
                    ...
                    ON CREATE SET d.label = 'Device-' + substring($device_id, 0, 8)
                    MERGE (s)-[rel:USES_DEVICE]->(d)
                    ON CREATE SET rel.since = date().toString()
                """, sender_id=sender_id, device_id=device_id)
            
            # 3. IP
            if ip_address:
                is_vpn = any(kw in str(ip_address).lower() for kw in ["vpn", "tor", "185.220", "47.135"])
                session.run("""
                    MATCH (s:Account {id: $sender_id})
                    MERGE (ip:IP {id: $ip_address})
                    ON CREATE SET ip.label = $ip_address, ip.is_vpn = $is_vpn
                    MERGE (s)-[rel:CONNECTS_FROM]->(ip)
                    ON CREATE SET rel.frequency = 'regular'
                """, sender_id=sender_id, ip_address=ip_address, is_vpn=is_vpn)
