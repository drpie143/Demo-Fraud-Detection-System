...
class Transaction(BaseModel):
    ...
    Features được enriched từ CSV:
    - device_id: UUID format (fe14f7ec-e97c-4acf-84d5-ca177182da22)
    - ip_address: Real IP (111.122.183.48)
    - geolocation: Lat/Long coordinates
    - sender_balance_before/after: Balance draining detection
    """
    ...
    amount: float = Field(..., description="Số tiền giao dịch theo currency")
    currency: str = Field(default="VND", description="Loại tiền tệ")
    transaction_type: str = Field(default="TRANSFER", description="Loại GD: TRANSFER/CASH_OUT/PAYMENT/CASH_IN")

    # Device & Network (from CSV)
    device_id: str = Field(default="", description="UUID thiết bị thực hiện GD")
    ip_address: str = Field(default="", description="Địa chỉ IP thực")
    channel: str = Field(default="mobile", description="Kênh: mobile/web/atm")

    # Geolocation (from CSV)
    geolocation_lat: Optional[float] = Field(default=None, description="Vĩ độ")
    geolocation_long: Optional[float] = Field(default=None, description="Kinh độ")
    location: str = Field(default="", description="Vị trí địa lý (text)")

    # Balance info (from CSV) - for drain detection
    ...
    expected_is_fraud: Optional[bool] = Field(default=None, description="Label benchmark nếu giao dịch đến từ final.csv")
