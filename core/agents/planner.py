...
from infrastructure.llm.gemini import gemini_provider_planner as gemini_provider
...
class PlannerAgent:
    ...
    def _parse_plan_response(self, response: dict) -> list[PlannerTask]:
        ...
        for raw in raw_tasks:
            ...
            sender_id = self.investigation_context.get("sender_id", "")
            receiver_id = self.investigation_context.get("receiver_id", "")
            if sender_id and sender_id not in description:
                description = (
                    f"{description}. Sender account: {sender_id}; "
                    f"receiver account: {receiver_id}; "
                    f"amount: {self.investigation_context.get('amount', 0)}."
                )
