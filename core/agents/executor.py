...
from core.schemas.models import PlannerTask, ExecutorResult, TaskType
...
from infrastructure.llm.gemini import gemini_provider_executor as gemini_provider
...
7. mongodb_history — Lấy lịch sử giao dịch gần đây
   params: {"account_id": "C8126703807", "limit": 20}
...
11. redis_velocity — Đếm tốc độ giao dịch (số lượng trong khoảng thời gian)
    params: {"account_id": "C8126703807", "hours": 1}
...
- Ví dụ: "HIGH_VELOCITY: 15 giao dịch trong 1h (baseline 2/h)", "SHARED_DEVICE: device_42 dùng chung với C2972777054", "KYC_NOT_VERIFIED: tài khoản C8126703807 status=pending"
...
class ExecutorAgent:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if not analysis.get("risk_indicators"):
                    analysis = self._fallback_analyze_results(task, tool_results)
    ...
    def _fallback_tool_calls(self, task: PlannerTask) -> list[dict]:
        ...
        import re
        found = re.findall(r'(?:ACC_\w+|MULE_\w+|[CM]\d{4,})', f"{task.description} {task.query}")
        acc_id = found[0] if found else ""

        fallbacks = {
            TaskType.ACCOUNT_PROFILING: [
                {"tool": "mongodb_profile", "params": {"account_id": acc_id}},
                {"tool": "mongodb_history", "params": {"account_id": acc_id, "limit": 20}},
                {"tool": "redis_velocity", "params": {"account_id": acc_id, "hours": 1}},
                {"tool": "redis_blacklist", "params": {"account_id": acc_id}},
            ],
    ...
    def _fallback_analyze_results(self, task: PlannerTask, results: list[dict]) -> dict:
        """Deterministic risk extraction used when Gemini is unavailable."""
        indicators: list[str] = []
        notes: list[str] = []

        def add(indicator: str):
        ...
        for result in results:
            ...
            elif tool == "mongodb_history" and isinstance(data, list):
                receivers = {txn.get("receiver_id") for txn in data if txn.get("receiver_id")}
                if len(data) > settings.suspicious_velocity_threshold:
                    add(f"HIGH_HISTORY_VOLUME: {len(data)} recent transactions")
                drained = [
                    txn for txn in data
                    if txn.get("sender_balance_before") not in (None, "")
                    and float(txn.get("sender_balance_before") or 0) > 0
                    and txn.get("sender_balance_after") is not None
                    and (
                        float(txn.get("sender_balance_after") or 0) <= 500
                        or (
                            float(txn.get("sender_balance_after") or 0)
                            / max(float(txn.get("sender_balance_before") or 0), 1.0)
                        ) <= 0.1
                    )
                ]
                if len(drained) >= 2:
                    add(f"BALANCE_DRAIN_HISTORY: {len(drained)} recent transactions left balance <= 500")
                if len(receivers) >= 5:
                    add(f"MULTI_RECIPIENT_PATTERN: sent to {len(receivers)} distinct receivers")
                notes.append(f"History: {len(data)} records, {len(receivers)} receivers")

            elif tool in {"neo4j_neighbors", "neo4j_blacklisted"}:
                records = []
                if isinstance(data, dict):
                    records = list(data.get("nodes", {}).items())
                elif isinstance(data, list):
                    records = [(item.get("account_id", ""), item) for item in data if isinstance(item, dict)]
                risky = [
                    node_id for node_id, info in records
                    if isinstance(info, dict) and info.get("risk") in {"critical", "high"}
                ]
                if risky:
                    add(f"RISKY_GRAPH_NEIGHBORS: connected to {', '.join(risky[:5])}")

            elif tool == "neo4j_shared_entities" and isinstance(data, list):
                suspicious = [item for item in data if item.get("is_suspicious")]
                if suspicious:
                    add(f"SHARED_INFRASTRUCTURE: {len(suspicious)} shared device/IP entities")

            elif tool == "redis_velocity" and isinstance(data, int):
                if data > settings.suspicious_velocity_threshold:
                    add(f"HIGH_VELOCITY: {data} transactions in 1h")

            elif tool == "redis_blacklist" and data is True:
                add("BLACKLISTED_ACCOUNT: account is in Redis blacklist")

        if not notes:
            notes.append(f"No deterministic risk evidence found for {task.task_type.value}.")

        return {
            "analysis": " ".join(notes[:4]),
