...
import json
from core.schemas.models import InvestigationReport, DecisionResult, FinalDecision
from infrastructure.databases.simulators import redis_service
from infrastructure.databases.chroma import vector_store
from infrastructure.llm.gemini import gemini_provider_detective as gemini_provider
...
class DetectiveAgent:
    ...
    def adjudicate(self, report: InvestigationReport, sender_id_fallback: str = "") -> DecisionResult:
        ...
        # ─── Gọi Gemini LLM ───
        if getattr(gemini_provider, "model", None) is None:
            llm_response = self._fallback_adjudication(report)
        else:
    ...
    def _fallback_adjudication(self, report: InvestigationReport) -> dict:
        """Rule-based final decision for offline simulator mode."""
        ...
        balance_drain_only = (
            len(report.risk_factors) == 1
            and "balance_drain" in risk_text
            and not critical_hits
        )

        if (
            report.recommended_decision == FinalDecision.BLOCK
            or critical_hits
            or (len(report.risk_factors) >= 3 and not balance_drain_only)
        ):
            return {
                "decision": "block",
                "confidence": max(report.confidence_score, 0.86),
                "reasoning": (
                    "Offline deterministic adjudication found strong fraud indicators: "
                    + ", ".join(report.risk_factors[:4])
                ),
                "risk_assessment": {
                    "critical": report.risk_factors[:5],
                    "high": [],
                    "medium": [],
                },
                "actions": ["block_transaction", "update_blacklist", "raise_risk_score"],
            }

        if report.recommended_decision == FinalDecision.ALLOW and not report.risk_factors:
            return {
                "decision": "allow",
                "confidence": max(report.confidence_score, 0.72),
                "reasoning": "Offline deterministic adjudication found no material risk indicators.",
                "risk_assessment": {"critical": [], "high": [], "medium": []},
                "actions": ["allow_transaction", "update_trust_profile"],
            }

        return {
            "decision": "escalate",
            "confidence": max(report.confidence_score, 0.55),
            "reasoning": "Offline deterministic adjudication did not reach sufficient confidence.",
            "risk_assessment": {"critical": [], "high": report.risk_factors[:5], "medium": []},
            "actions": ["hold_transaction", "notify_human_reviewer"],
        }
